...
export const CommandAnimation: FunctionComponent<Props> = ({ isStarted }) => {
    ...
    useEffect(() => {
        ...
        let blinkIndex = 0, _charIndex = charIndex; // inital setup
        ...
        const interval = setInterval(() => {
            ...
            if (_charIndex === title.length) {
                ...
                if (blinkIndex < maxBlink - 1) {
                    setText([title.substring(0, _charIndex), blinkIndex % 2 ? "_" : ""].join(''))
                    blinkIndex++;
    ...
    return <Box color="grey" fontWeight="lighter">{text}</Box>
}