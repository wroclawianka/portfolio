import { Box, Button, Flex, Grid, GridItem, Icon, IconButton, Image, Link, List, ListItem, Text, useMultiStyleConfig } from '@chakra-ui/react';
    ...
    <Box sx={style.root}>
          ...
          <GridItem key={title}>
            <Box sx={style.project}>
              <Flex flexDirection="column" justifyContent="space-between" height="100%">
                <Box>
                  <Flex sx={style.flex}>
                    <Text alignSelf={"center"} fontWeight="bold" fontSize='lg'>{title}</Text>
                ...
                <Box>
                  <Text pt={4} fontSize='md'>
                    <List display="flex" gap={3}>
                      {stack.map(element => (<ListItem key={element}>{element}</ListItem>))}
                    </List>
                  </Text>
                </Box>
              </Flex>
      ...
      <Flex mt={6} justifyContent="center">
        {projects.length !== list.length ?
          <Button size="lg" colorScheme="teal" onClick={() => seeMore()}>See more</Button> : <></>}
      </Flex>
    </Box>
  );
};
